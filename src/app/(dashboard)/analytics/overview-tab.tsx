...
export default function OverviewTab() {
  ...
  const generateInsights = (
    ...
    // 1. Unlucky teams (high PF, bad record)
    const pfRanked = [...standings].sort((a, b) => b.pts_for - a.pts_for);
    pfRanked.forEach((team) => {
      const pfRank = pfRanked.indexOf(team) + 1;
      if (pfRank <= 5 && team.league_rank > 6) {
        ins.push(`${team.team_name} has the ${ordinal(pfRank)} highest points-for but sits ${ordinal(team.league_rank)} on the ladder — the unluckiest schedule in the league?`);
      }
      if (pfRank >= 7 && team.league_rank <= 4) {
        ins.push(`${team.team_name} is ${ordinal(team.league_rank)} on the ladder despite having only the ${ordinal(pfRank)} highest points-for — riding schedule luck.`);
      }
    });

    // 2. Over/under-ranked in PWRNKGs
    ...
    // 4. Similar PF different records
    for (let i = 0; i < standings.length; i++) {
      for (let j = i + 1; j < standings.length; j++) {
        const a = standings[i], b = standings[j];
        const pfDiff = Math.abs(a.pts_for - b.pts_for);
        const winDiff = Math.abs(a.wins - b.wins);
        if (pfDiff < 300 && winDiff >= 3) {
          ins.push(`${a.team_name} and ${b.team_name} have nearly identical points-for (${formatScore(a.pts_for)} vs ${formatScore(b.pts_for)}) but vastly different records (${a.wins}-${a.losses} vs ${b.wins}-${b.losses}).`);
        }
      }
    }
